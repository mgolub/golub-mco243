...
public class SelfishRoundRobinScheduler extends Scheduler {
	...
	public static void main(String[] args) {
		List<Job> jobs = Arrays.asList(new Job("1", Priority.High, JobType.Computation, 100, 5L),
				new Job("2", Priority.Low, JobType.Computation, 200, 3L),
				new Job("3", Priority.Medium, JobType.IO, 100, 2L),
				new Job("4", Priority.Medium, JobType.Computation, 1000, 4L),
				new Job("5", Priority.High, JobType.Computation, 350, 6L),
				new Job("6", Priority.Low, JobType.IO, 20, 1L), new Job("7", Priority.High, JobType.IO, 30, 4L),
				new Job("8", Priority.Medium, JobType.Computation, 600, 5L),
				new Job("9", Priority.High, JobType.Computation, 700, 6L),
				new Job("10", Priority.Low, JobType.IO, 200, 3L));

		SelfishRoundRobinScheduler scheduler = new SelfishRoundRobinScheduler(new ArrayList<Job>(jobs));
